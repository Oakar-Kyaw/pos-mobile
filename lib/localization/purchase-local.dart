mixin PurchaseLocale {
  static const purchaseManagementTitle = 'purchase_management_title';
  static const purchaseList = 'purchase_list';
  static const purchaseCreate = 'purchase_create';
  static const purchaseEdit = 'purchase_edit';
  static const purchaseUpdate = 'purchase_update';
  static const purchaseDelete = 'purchase_delete';
  static const purchaseCancel = 'purchase_cancel';
  static const purchaseDetail = 'purchase_detail';
  static const purchaseSupplier = 'purchase_supplier';
  static const purchaseSupplierStatus = 'purchase_supplier_status';
  static const purchaseSelectSupplier = 'purchase_select_supplier';
  static const purchaseOrderDate = 'purchase_order_date';
  static const purchaseReceivedDate = 'purchase_received_date';
  static const purchaseStatus = 'purchase_status';
  static const purchaseQuantity = 'purchase_quantity';
  static const purchasePrice = 'purchase_price';
  static const purchaseUnitPrice = 'purchase_unit_price';
  static const purchaseSubtotal = 'purchase_subtotal';
  static const purchaseItemTotal = 'purchase_item_total';
  static const purchaseGrandTotal = 'purchase_grand_total';
  static const purchaseTax = 'purchase_tax';
  static const purchaseDiscount = 'purchase_discount';
  static const purchaseDiscountPercent = 'purchase_discount_percent';
  static const purchaseDeliveryFee = 'purchase_delivery_fee';
  static const purchaseDeliveryPercent = 'purchase_delivery_percent';
  static const purchasePackagingFee = 'purchase_packaging_fee';
  static const purchaseTotalAmount = 'purchase_total_amount';
  static const purchaseInvalidAmount = 'purchase_invalid_amount';
  static const purchasePayment = 'purchase_payment';
  static const purchaseAddPayment = 'purchase_add_payment';
  static const purchasePaymentAmount = 'purchase_payment_amount';
  static const purchasePaymentMethod = 'purchase_payment_method';
  static const purchasePaymentDate = 'purchase_payment_date';
  static const purchasePaymentStatus = 'purchase_payment_status';
  static const purchasePaidAmount = 'purchase_paid_amount';
  static const purchaseRemainingAmount = 'purchase_remaining_amount';
  static const purchasePaymentRequired = 'purchase_payment_required';
  static const purchasePaymentAmountRequired =
      'purchase_payment_amount_required';
  static const purchaseInvalidPaymentAmount = 'purchase_invalid_payment_amount';
  static const purchaseAddItem = 'purchase_add_item';
  static const purchaseSubmit = 'purchase_submit';
  static const purchasePending = 'purchase_pending';
  static const purchaseReceived = 'purchase_received';
  static const purchaseCancelled = 'purchase_cancelled';
  static const purchaseDeliveredSuccessfully =
      'purchase_delivered_successfully';
  static const purchaseNote = 'purchase_note';
  static const purchaseCard = 'purchase_card';
  static const purchaseItemCard = 'purchase_item_card';
  static const purchaseGoToInventory = 'purchase_go_to_inventory';
  static const purchaseError = 'purchase_error';
  static const purchaseSuccess = 'purchase_success';
  static const purchaseCreateSuccess = 'purchase_create_success';
  static const purchaseUpdateSuccess = 'purchase_update_success';
  static const purchaseDeleteSuccess = 'purchase_delete_success';
  static const purchaseUpdateFail = 'purchase_update_fail';
  static const purchaseDeleteFail = 'purchase_delete_fail';
  static const purchaseUpdateConfirm = 'purchase_update_confirm';
  static const purchaseDeleteConfirm = 'purchase_delete_confirm';
  static const purchaseSupplierRequired = 'purchase_supplier_required';
  static const purchaseStatusRequired = 'purchase_status_required';
  static const purchaseOrderDateRequired = 'purchase_order_date_required';
  static const purchaseItemRequired = 'purchase_item_required';

  static const EN = {
    purchaseManagementTitle: 'Purchase Management',
    purchaseList: 'Purchase Records',
    purchaseCreate: 'Create Purchase',
    purchaseEdit: 'Edit',
    purchaseUpdate: 'Update Purchase',
    purchaseDelete: 'Delete Purchase',
    purchaseCancel: 'Cancel',
    purchaseDetail: 'Detail',
    purchaseSupplier: 'Supplier',
    purchaseSupplierStatus: 'Supplier Status',
    purchaseSelectSupplier: 'Select Supplier',
    purchaseOrderDate: 'Order Date',
    purchaseReceivedDate: 'Received Date',
    purchaseStatus: 'Purchase Status',
    purchaseQuantity: 'Quantity',
    purchasePrice: 'Price',
    purchaseUnitPrice: 'Unit Price',
    purchaseSubtotal: 'Subtotal',
    purchaseItemTotal: 'Item Total',
    purchaseGrandTotal: 'Grand Total',
    purchaseTax: 'Tax',
    purchaseDiscount: 'Discount',
    purchaseDiscountPercent: 'Discount %',
    purchaseDeliveryFee: 'Delivery Fee',
    purchaseDeliveryPercent: 'Delivery %',
    purchasePackagingFee: 'Packaging Fee',
    purchaseTotalAmount: 'Total Amount',
    purchaseInvalidAmount: 'Invalid amount',
    purchasePayment: 'Payment',
    purchaseAddPayment: 'Add Payment',
    purchasePaymentAmount: 'Payment Amount',
    purchasePaymentMethod: 'Payment Method',
    purchasePaymentDate: 'Payment Date',
    purchasePaymentStatus: 'Payment Status',
    purchasePaidAmount: 'Paid Amount',
    purchaseRemainingAmount: 'Remaining Amount',
    purchasePaymentRequired: 'Please add a payment',
    purchasePaymentAmountRequired: 'Please enter payment amount',
    purchaseInvalidPaymentAmount: 'Invalid payment amount',
    purchaseAddItem: 'Add Item',
    purchaseSubmit: 'Submit',
    purchasePending: 'Pending',
    purchaseReceived: 'Received',
    purchaseCancelled: 'Cancelled',
    purchaseDeliveredSuccessfully: 'Purchase delivered successfully',
    purchaseNote: 'Note',
    purchaseCard: 'Purchase Records',
    purchaseItemCard: 'Purchase Items',
    purchaseGoToInventory: 'Go to Inventory',
    purchaseError: 'Something went wrong',
    purchaseSuccess: 'Success',
    purchaseCreateSuccess: 'Purchase created successfully',
    purchaseUpdateSuccess: 'Purchase record updated successfully',
    purchaseDeleteSuccess: 'Purchase record deleted successfully',
    purchaseUpdateFail: 'Failed to update purchase record',
    purchaseDeleteFail: 'Failed to delete purchase record',
    purchaseUpdateConfirm: 'Do you want to update this purchase record?',
    purchaseDeleteConfirm: 'Do you want to delete this purchase record?',
    purchaseSupplierRequired: 'Please select a supplier',
    purchaseStatusRequired: 'Please select a purchase status',
    purchaseOrderDateRequired: 'Please select an order date',
    purchaseItemRequired: 'Please add at least one item',
  };

  static const MM = {
    purchaseManagementTitle: 'အဝယ်စီမံခန့်ခွဲမှု',
    purchaseList: 'အဝယ်မှတ်တမ်းများ',
    purchaseCreate: 'ဖန်တီးရန်',
    purchaseEdit: 'ပြင်ဆင်ရန်',
    purchaseUpdate: 'ပြင်ဆင်ရန်',
    purchaseDelete: 'ဖျက်ရန်',
    purchaseCancel: 'ပယ်ဖျက်ရန်',
    purchaseDetail: 'အသေးစိတ်',
    purchaseSupplier: 'ပေးသွင်းသူ',
    purchaseSupplierStatus: 'ပေးသွင်းသူ အခြေအနေ',
    purchaseSelectSupplier: 'ပေးသွင်းသူ ရွေးချယ်ပါ',
    purchaseOrderDate: 'မှာယူသည့်ရက်',
    purchaseReceivedDate: 'လက်ခံရရှိသည့်ရက်',
    purchaseStatus: 'အဝယ်အခြေအနေ',
    purchaseQuantity: 'အရေအတွက်',
    purchasePrice: 'ဈေးနှုန်း',
    purchaseUnitPrice: 'တစ်ခုချင်းဈေးနှုန်း',
    purchaseSubtotal: 'စုစုပေါင်းတန်ဖိုး',
    purchaseItemTotal: 'ပစ္စည်းစုစုပေါင်း',
    purchaseGrandTotal: 'နောက်ဆုံးစုစုပေါင်း',
    purchaseTax: 'အခွန်',
    purchaseDiscount: 'လျှော့စျေး',
    purchaseDiscountPercent: 'လျှော့စျေး ရာခိုင်နှုန်း',
    purchaseDeliveryFee: 'ပို့ဆောင်ခ',
    purchaseDeliveryPercent: 'ပို့ဆောင်ခ ရာခိုင်နှုန်း',
    purchasePackagingFee: 'ထုပ်ပိုးခ',
    purchaseTotalAmount: 'စုစုပေါင်းကျသင့်ငွေ',
    purchaseInvalidAmount: 'မမှန်ကန်သော ပမာဏဖြစ်ပါသည်',
    purchasePayment: 'ငွေပေးချေမှု',
    purchaseAddPayment: 'ငွေပေးချေမှု ထည့်ရန်',
    purchasePaymentAmount: 'ပေးချေသည့်ပမာဏ',
    purchasePaymentMethod: 'ငွေပေးချေမှုနည်းလမ်း',
    purchasePaymentDate: 'ငွေပေးချေသည့်ရက်',
    purchasePaymentStatus: 'ငွေပေးချေမှု အခြေအနေ',
    purchasePaidAmount: 'ပေးချေပြီးငွေ',
    purchaseRemainingAmount: 'ကျန်ရှိငွေ',
    purchasePaymentRequired: 'ကျေးဇူးပြု၍ ငွေပေးချေမှု ထည့်ပါ',
    purchasePaymentAmountRequired: 'ကျေးဇူးပြု၍ ပေးချေမည့်ပမာဏကို ထည့်ပါ',
    purchaseInvalidPaymentAmount: 'မမှန်ကန်သော ငွေပေးချေမှုပမာဏ ဖြစ်ပါသည်',
    purchaseAddItem: 'ပစ္စည်းထည့်ရန်',
    purchaseSubmit: 'တင်သွင်းမည်',
    purchasePending: 'စောင့်ဆိုင်းနေသည်',
    purchaseReceived: 'လက်ခံရရှိပြီး',
    purchaseCancelled: 'ပယ်ဖျက်ပြီး',
    purchaseDeliveredSuccessfully:
        'အဝယ်ပစ္စည်းများကို အောင်မြင်စွာ လက်ခံရရှိပြီးပါပြီ',
    purchaseNote: 'မှတ်ချက်',
    purchaseCard: 'အဝယ်မှတ်တမ်းများ',
    purchaseItemCard: 'အဝယ်ပစ္စည်းများ',
    purchaseGoToInventory: 'Inventory သို့ သွားမည်',
    purchaseError: 'အမှားတစ်ခုဖြစ်ပွားခဲ့သည်',
    purchaseSuccess: 'အောင်မြင်ပါသည်',
    purchaseCreateSuccess: 'အဝယ်ကို အောင်မြင်စွာ ဖန်တီးပြီးပါပြီ',
    purchaseUpdateSuccess: 'အဝယ်မှတ်တမ်းကို အောင်မြင်စွာ ပြင်ဆင်ပြီးပါပြီ',
    purchaseDeleteSuccess: 'အဝယ်မှတ်တမ်းကို အောင်မြင်စွာ ဖျက်ပြီးပါပြီ',
    purchaseUpdateFail: 'အဝယ်မှတ်တမ်း ပြင်ဆင်ရန် မအောင်မြင်ပါ',
    purchaseDeleteFail: 'အဝယ်မှတ်တမ်း ဖျက်ရန် မအောင်မြင်ပါ',
    purchaseUpdateConfirm: 'ဤအဝယ်မှတ်တမ်းကို ပြင်ဆင်လိုပါသလား?',
    purchaseDeleteConfirm: 'ဤအဝယ်မှတ်တမ်းကို ဖျက်လိုပါသလား?',
    purchaseSupplierRequired: 'ကျေးဇူးပြု၍ ပေးသွင်းသူကို ရွေးချယ်ပါ',
    purchaseStatusRequired: 'ကျေးဇူးပြု၍ အဝယ်အခြေအနေကို ရွေးချယ်ပါ',
    purchaseOrderDateRequired: 'ကျေးဇူးပြု၍ မှာယူသည့်ရက်ကို ရွေးချယ်ပါ',
    purchaseItemRequired: 'အနည်းဆုံး ပစ္စည်းတစ်ခု ထည့်ပါ',
  };
}
